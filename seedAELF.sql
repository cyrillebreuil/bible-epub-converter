BEGIN;
    ...
    ON CONFLICT ("isNewTestament", "translationID") DO NOTHING;
      ...
      ON CONFLICT ("bookID", "translationID") DO NOTHING;
                   ...
                   (SELECT "id" FROM "translations" WHERE "code" = 'aelf'),
                   25,
                   'Adam s’unit encore à sa femme, et elle mit au monde un fils. Elle lui donna le nom de')
                   ON CONFLICT ("chapterID", "translationID", "number") DO NOTHING;
                   ...
                   (SELECT "id" FROM "translations" WHERE "code" = 'aelf'),
                   3,
                   'Adam vécut cent trente ans, puis il engendra un fils à sa ressemblance et selon son image; il l’appela du nom de')
                   ON CONFLICT ("chapterID", "translationID", "number") DO NOTHING;
